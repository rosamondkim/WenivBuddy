...
import { applyMapping, getStandardTerm } from './keyword-mapping.js'
import { extractKeywordsWithLLM } from './llm-keyword-extractor.js'
...
export async function extractKeywordsHybrid(question, options = {}) {
  ...
  // OCR 텍스트의 경우 신뢰도 임계값을 높임 (더 엄격한 기준)
  // 하지만 무조건 LLM을 강제하지는 않고, 높은 신뢰도면 로컬 사용
  const effectiveThreshold = isOCR ? 0.8 : confidenceThreshold
  const shouldForceLLM = forceLLM

  if (isOCR) {
    console.log(`🖼️ [Hybrid] OCR mode - using higher threshold (${Math.round(effectiveThreshold * 100)}%)`)
  }
  ...
  if (!shouldForceLLM && confidence >= effectiveThreshold) {
    ...
    if (llmResult && llmResult.keywords.length > 0) {
      // LLM 성공
      // OCR 모드일 경우 로컬 키워드도 병합 (에러 메시지 등 특수 텍스트를 놓치지 않기 위해)
      let finalKeywords = llmResult.keywords
      if (isOCR) {
        // 로컬 키워드와 LLM 키워드 병합
        finalKeywords = [...new Set([...llmResult.keywords, ...combinedKeywords])].slice(0, 20)
      }
