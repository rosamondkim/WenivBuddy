import { calculateSimilarity, countKeywordMatches } from './keyword-extractor.js';
import { extractKeywordsHybrid } from './hybrid-keyword-extractor.js';
 ...
 * @param {number} minSimilarity - 최소 유사도 임계값 (기본값: 0.1, 즉 10%)
 * @param {boolean} isOCR - OCR 텍스트 여부 (기본값: false)
 * @returns {Promise<Object>} - 검색 결과 및 추출 정보
 */
export async function searchQnA(qnaList, searchQuery, category = 'all', maxResults = 3, minSimilarity = 0.1, isOCR = false) {
  ...
  const scoredResults = filteredList.map(qna => {
    ...
    if (isOCR) {
      ...
      // 일반 텍스트 검색일 경우
      // 짧은 검색어(키워드 3개 이하)는 매칭 가중치를 더 높게
      ...
      if (isShortQuery) {
        ...
        totalScore =
          ...
          (titleMatches / keywordCount * 0.7) +
          (bodyMatches / keywordCount * 0.15) +
          (answerMatches / keywordCount * 0.05);
      } else {
        // 긴 검색어: 기존 로직 유지
        totalScore =
  ...
  // 4. 점수 순으로 정렬 (높은 점수부터)
  // OCR 검색 시에는 임계값을 더 낮춤 (에러 메시지 매칭을 위해)
  const effectiveMinSimilarity = isOCR ? Math.min(minSimilarity, 0.05) : minSimilarity

  const sortedResults = scoredResults
    .filter(result => result.score >= effectiveMinSimilarity) // 최소 유사도 임계값 이상만 포함
